    ...
    This lesson focuses on how to choose the right collection for small retrieval and ranking tasks. The goal is not to
    memorize syntax. The goal is to recognize which Python construct gives the
    next AI component a cleaner contract.

    - `lists`: a Python tool this lesson uses to choose the right collection for small retrieval and ranking tasks.
- `tuples`: a Python tool this lesson uses to choose the right collection for small retrieval and ranking tasks.
- `sets`: a Python tool this lesson uses to choose the right collection for small retrieval and ranking tasks.
- `dicts`: a Python tool this lesson uses to choose the right collection for small retrieval and ranking tasks.
- `Counter`: a Python tool this lesson uses to choose the right collection for small retrieval and ranking tasks.
- `defaultdict`: a Python tool this lesson uses to choose the right collection for small retrieval and ranking tasks.
    ...
    - `retrieval_collections.py` contains the focused implementation.
    - `demo.py` runs the main workflow from the command line.
    - `test_retrieval_collections.py` proves the important behavior and failure modes.
    ...
    ```bash
    cd 01_03_python-data-structures-deep-dive
    python3 demo.py
    python3 -m pytest
    ```
