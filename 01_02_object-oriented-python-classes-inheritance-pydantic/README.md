    # Object-Oriented Python: Classes, Inheritance & Pydantic
    ...
    This lesson focuses on how to model agent requests and responses with explicit validation boundaries. The goal is not to
    memorize syntax. The goal is to recognize which Python construct gives the
    next AI component a cleaner contract.

    - `classes`: a Python tool this lesson uses to model agent requests and responses with explicit validation boundaries.
- `dataclasses`: a Python tool this lesson uses to model agent requests and responses with explicit validation boundaries.
- `inheritance`: a Python tool this lesson uses to model agent requests and responses with explicit validation boundaries.
- `validation`: a Python tool this lesson uses to model agent requests and responses with explicit validation boundaries.
- `Pydantic-style schemas`: a Python tool this lesson uses to model agent requests and responses with explicit validation boundaries.
    ...
    - `agent_models.py` contains the focused implementation.
    - `demo.py` runs the main workflow from the command line.
    - `test_agent_models.py` proves the important behavior and failure modes.
    ...
    ```bash
    cd 01_02_object-oriented-python-classes-inheritance-pydantic
    python3 demo.py
    python3 -m pytest
    ```
