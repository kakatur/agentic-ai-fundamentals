    # Python Basics for AI: Variables, Types & Control Flow
    ...
    This lesson focuses on how to turn messy user text into a small structured request that later AI code can trust. The goal is not to
    memorize syntax. The goal is to recognize which Python construct gives the
    next AI component a cleaner contract.

    - `variables`: a Python tool this lesson uses to turn messy user text into a small structured request that later AI code can trust.
- `types`: a Python tool this lesson uses to turn messy user text into a small structured request that later AI code can trust.
- `branches`: a Python tool this lesson uses to turn messy user text into a small structured request that later AI code can trust.
- `loops`: a Python tool this lesson uses to turn messy user text into a small structured request that later AI code can trust.
- `functions`: a Python tool this lesson uses to turn messy user text into a small structured request that later AI code can trust.
    ...
    - `ai_text_pipeline.py` contains the focused implementation.
    - `demo.py` runs the main workflow from the command line.
    - `test_ai_text_pipeline.py` proves the important behavior and failure modes.
    ...
    ```bash
    cd 01_01_python-basics-for-ai-variables-types-control-flow
    python3 demo.py
    python3 -m pytest
    ```
